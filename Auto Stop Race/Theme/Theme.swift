//
//  Theme.swift
//  Auto Stop Race
//
//  Created by RI on 03/03/2018.
//  Copyright © 2018 Torianin. All rights reserved.
//

import UIKit

struct Theme {
    struct Color {
        static let blueMenu = #colorLiteral(red: 0.09803921569, green: 0.462745098, blue: 0.8235294118, alpha: 1)
        static let grayBackground = #colorLiteral(red: 0.8078431373, green: 0.8078431373, blue: 0.8235294118, alpha: 1)
        static let success = #colorLiteral(red: 0.3607843137, green: 0.7215686275, blue: 0.3607843137, alpha: 1)
        static let circleColor = #colorLiteral(red: 0.8352941176, green: 0, blue: 0, alpha: 1)
        static let grayBackgroundColor = #colorLiteral(red: 0.968627451, green: 0.968627451, blue: 0.968627451, alpha: 1)
    }
}
