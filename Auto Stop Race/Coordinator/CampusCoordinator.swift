...
final class CampusCoordinator: Coordinator {
    ...
    func start() {
        guard let url = URL(string: ApiConfig.assetsJsonUrl),
            ...
            let campusMapImageUrl = URL(string: raceInfoImages.campusMapImageUrl) else { return }
        var images = [SKPhoto]()
        KingfisherManager.shared.retrieveImage(with: campusMapImageUrl, options: nil, progressBlock: nil) { result in
            switch result {
